...
static bool TryGetCount(const Aws::Map<Aws::String, Aws::String> &parameters, const char *key, idx_t &result) {
	...
	uint64_t count;
	if (!TryCast::Operation(string_t(entry->second.c_str(), UnsafeNumericCast<uint32_t>(entry->second.size())), count,
	                        true)) {
		return false;
	}
	result = count;
